...
- <b>[IO]</b> is intended to simplify the process of integrating various command line tools, by providing a single interface instead of many.

- <b>[IO]</b> is highly customizable and scalable, allowing users to create a tool to best suite their individual needs.

- <b>[IO]</b> is extremely light-weight, and only requires an operating system and file system to run.
...
- <b>[IO]</b> will only detect the stdout (standard output) of an executable:

  - If your command line tool has complex nested processes and subprocesses, <b>[IO]</b> will only print to console what it is provided through stdout. This is the intended behavior.

- Settings and usage information for a command line tool are the responsibility of that tool:

  - <b>[IO]</b> will primarily pass the provided command line arguments to the command line tool that has been called as a subcommand.

  - If a user wants to change the settings or view the usage for a given command line tool, it is the responsibility of that tool to maintain these features.

- <b>[IO]</b> works best with executables that do not require being run with other programs or software.
  
  - For scripting languages, an easy way around this is to use the appropriate shebang for that scripting language. However, we understand this can sometimes be easier said than done when it comes to large, complex programs.
  
  - We recommend wrapping these executables in a bash or zsh script that performs this step instead:
    ...
    - By doing so, the bash/zsh script itself can be used as the executable that <b>[IO]</b> will run as a subcommand. This retains all functionality from the original program and allows it to integrate nicely with <b>[IO]</b>.
